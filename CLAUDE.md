...
- `leaderboards?contestId=X&slateId=Y&picksSlateId=Y&limit=150` (no `entryId`) — every entry's picks and metrics for week Y in one call (`fetchSlateWeeks` in `src/picks.ts`, matched to our filtered handles locally). Each pick carries `team`, `spread`, `grade`, `effectivePoints`. `grade` has **five** observed values, not three: `"won"`/`"lost"`/`"push"` (final) and `"winning"`/`"losing"` — Splash's own live read on any in-progress game's pick, not something specific to the tiebreaker game (confirmed on a regular game too, despite an earlier wrong assumption here). `null` means not yet started/graded. **Adding `&entryId=Z` to scope to one user returns a thinner payload** — no `picks`, no `potential_points` in `metrics` — discovered the hard way after an earlier version used the per-entry variant and got `NaN` pending counts; fetch the whole slate and filter locally instead of adding `entryId` back.
- Same `leaderboards` endpoint, no `slateId` at all (just `contestId`+`userId`) — a season-level single-user lookup, also missing `picks` and `potential_points`. `picks`/`potential_points` only show up once a slate context (`slateId`+`picksSlateId`) is given.
- Every leaderboard entry (season-wide or per-slate) also carries `tiebreaker.steps`, a `team_pick_em_cascade` array (`wins` → `last_game_total` → `points`). The `last_game_total` step's `total` is **already Splash's own precomputed guess-the-score tiebreaker diff** (confirmed identical to that same entry's `metrics[]` row labeled `"TieDiff"`) — proven by the fact that `total` varies per entry even though it's the same tiebreaker game for everyone, which a raw score never would. Don't subtract the step's `submitted` field (the user's raw guess) from it — `toStandingsEntry` in `src/splash.ts` learned this the hard way (an earlier version computed `|total - submitted|`, silently double-transforming an already-correct value). `total` is `null` until the tiebreaker game is decided.
...
`refreshWeeklyCache()` in `index.ts` **only fetches slates that have started** (`status !== "scheduled"`, or `isCurrentSlate`) and, once a slate's `settledDate` is set, caches it as `final` and never refetches it — without this the Worker would re-fetch pick data for every one of ~20 weeks on every cron tick for the entire season. It fetches each active slate's whole leaderboard once (`fetchSlateWeeks`) rather than once per filtered handle — see above for why.

`pending` (shown on the Overall tab) and the weekly `Score`/`Pending` columns all come from this same weekly data — no separate fetch. **`computePendingCount` deliberately uses `week.potentialPoints - week.wins` (Splash's own `metrics[].value` where `type === "potential_points"` — the max score reachable if every still-undecided pick comes in as a win) rather than counting pick grades itself, or even our own `picksRequiredCount - decided` math.** An earlier version filtered `picks` for "final" grades and undercounted pending by one per user once "winning"/"losing" showed up, since that grade isn't null but also isn't final; a later version used `picksRequiredCount - (wins+losses+ties)`, which is equivalent but needlessly depends on `picksRequiredCount`. Splash's own `potential_points` is correct by construction regardless of what grade strings exist, including ones not discovered yet — this is the second time an unanticipated Splash value (`"finished"` was the first) broke code that tried to enumerate all possible values instead of trusting Splash's own aggregate. Don't reintroduce pick-grade counting or `picksRequiredCount`-based math here.
...
- `computeWeeklyPending` in `lib.js` **must** stay in sync with `computePendingCount` in `src/picks.ts` — same `potentialPoints - wins` logic, intentionally duplicated because the frontend and Worker are separate runtimes with no shared module boundary. An earlier version of the frontend copy used the old (wrong) pick-grade-counting approach after the backend had already been fixed — check both when touching either.
- Live (ungraded) picks are colored by whether they're *currently covering the spread* (score margin + spread, mirroring how the final `grade` works), not a preview of Splash's own eventual grade. This is `coverStatus` in `lib.js` (previously named `livePickClass` and live-games-only; generalized to also compute won/lost/push for a *finished* game purely from score+spread) — `pickCellClass` prefers Splash's real per-pick `grade` when present and falls back to `coverStatus` otherwise. That fallback path is the *only* path available for pool-wide picks (the Trends tab's "Pool" sections), since we never have a per-pick `grade` for the ~120 entrants outside `FILTERED_HANDLES` — only their team+spread, from the shared game catalog.
...
- **Trends tab** (`renderGroupTrends`/`renderPoolWeekTrends`/`renderPoolSeasonTrends` in `app.js`): "Our Group" surfaces only interesting games from `computeGroupTrends` — a consensus (>= 3 of our filtered users on the same team, threshold configurable) or a head-to-head split (our users on opposite sides). Each game renders as one compact row: a horizontal bar split into one segment per team, sized by pick count via flexbox (`flex: <count> 1 0%` on the label/segment/handles-cell triplet for each team, all three keyed off the same count so they stay aligned) — a 3-1 split reads as a 75/25 bar, not two equal-looking boxes. This replaced an earlier two-column "vs" card after the user found it too text-heavy/scroll-heavy for a week with several interesting games; the card gave a lone contrarian pick the same visual weight as the group's consensus, which the proportional bar fixes directly. "Pool" sections use `computePoolTopPicks`/`computeSeasonPoolTopTeams` to rank pick popularity across the *whole* contest (not just filtered users) as a similar magnitude-bar list (`.meter-*` CSS classes) rather than a table. Bar segment colors are each team's live/final status (`--good`/`--bad`/neutral, via `pickCellClass`) paired with a small icon (`trendStatusIcon` in `app.js`) so status never rides on color alone; the Pool bars' blue (`--accent`) is intentionally a different hue from `--good`/`--bad` so "how popular" is never confused with "won or lost." Season-long pool trends get no status color at all, since a team plays a different game (and side) each week so "won or lost" has no single answer across a season.
...
`fetchSlateWeeks` (`test/picks.test.ts`) is mocked against the real API shape using `cloudflare:test`'s `fetchMock` (an undici `MockAgent`: `fetchMock.activate()` + `fetchMock.disableNetConnect()` in `beforeAll`, `fetchMock.get(origin).intercept({ path, method }).reply(...)` per test, `fetchMock.assertNoPendingInterceptors()` in `afterEach`) — this is the pattern to copy for any other real-API function you add tests for. This test exists *because* an earlier version's assumption about the response shape was wrong in production (see the `entryId` note above) and nothing caught it; don't let that regress.

Still **not** tested, as of this writing: the success path of `POST /api/refresh-token` and the other functions in `src/*.ts` that call the real Splash API (`fetchSlates`, `fetchGameCatalog`, `fetchFullLeaderboard`, `getValidIdToken`) or `src/index.ts`'s orchestration functions (`refreshAll`, `refreshWeeklyCache`'s final-slate caching logic, `computeAggregatesCache`) — none of these are exported. Use the `fetchMock` pattern above rather than hitting Splash's real API from a test.
